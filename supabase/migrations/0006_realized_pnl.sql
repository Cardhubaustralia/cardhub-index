-- ============================================================
-- CardHub Index — realized P&L on sells (for transaction history)
-- Adds orders.realized_pnl and recomputes it inside execute_cycle.
-- Re-run of 0002's execute_cycle with the P&L line added.
-- ============================================================
...
begin
  ...
  loop
    ...
    if r.side = 'buy' then
      ...
      v_cost := v_price * r.qty;
      v_pnl := (v_price - coalesce(v_avg, 0)) * r.qty;   -- realized gain/loss
      update portfolios set cash = cash + v_cost where id = r.portfolio_id;
