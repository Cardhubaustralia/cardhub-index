...
export default async function Dashboard() {
  ...
  const [{ count: pricedCount }, { data: hero }, { data: gainers }, { data: losers }, { data: mostTraded }] =
    await Promise.all([
      supabase.from("assets").select("id", { count: "exact", head: true }).not("price", "is", null),
      supabase
        .from("v_movers").select("*")
        .order("change_7d_pct", { ascending: false, nullsFirst: false })
        .limit(4),
      supabase
        .from("v_movers").select("*")
        .order("change_7d_pct", { ascending: false, nullsFirst: false })
      ...
      supabase
        .from("v_movers").select("*")
        .order("change_7d_pct", { ascending: true, nullsFirst: false })
        .limit(6),
      supabase
        .from("v_most_traded").select("*")
          ...
          {!!hero?.length && (
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Link href="/market?sort=change-desc" className="text-sm font-extrabold text-blue-600 hover:underline">
                  See all movers →
                </Link>
              </div>
